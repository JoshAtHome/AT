...
att_estimate <- function(heterogeneity, violation, x_formula){
  ...
  att_est <- att$att[5]                                                         # estimated treatment effect
  att_act <- mean(dat$effect[dat$treat == 1])                                   # actual treatment effect
  beta <- att$att                                                               # atts
  sigma <- as.matrix(att$V_analytical)                                          # var-cov-matrix
  return(list(att_est=att_est, att_act=att_act, p=p, beta=beta,sigma=sigma))
}
...
sim <- function(amount, conditional, heterogeneity, violation) {
  ...
  p_values <- numeric(amount)                                                   # all pre-treatment trends test results
  ...
  return(list(p_values=p_values, bias=bias))
}
